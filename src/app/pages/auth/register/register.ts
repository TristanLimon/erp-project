...
<div class="register-wrap">
  <div class="register-card">
    ...
    <p class="reg-sub">Completa tus datos para registrarte en el sistema</p>
    ...
    <p-button label="Crear cuenta" icon="pi pi-user-plus"
              styleClass="w-full mt-4 justify-content-center"
  ...
  async register() {
    ...
    if (!nombreCompleto || !usuario || !email || !password) {
      this.msg.add({
        ...
        detail: 'Nombre, usuario, email y contraseña son obligatorios.',
      });
      return;
    }

    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      this.msg.add({ severity: 'warn', summary: 'Email inválido', detail: 'Ingresa un correo válido.' });
      return;
    }

    if (password.length < 6) {
      this.msg.add({ severity: 'warn', summary: 'Contraseña corta', detail: 'Mínimo 6 caracteres.' });
      return;
    }

    this.loading = true;
    try {
      const result = await this.ps.registrar({
        email,
        password,
        usuario,
        nombreCompleto,
      ...
      if (result.ok) {
        this.msg.add({
          ...
          detail: 'Revisa tu correo para confirmar tu cuenta, luego inicia sesión.',
          life: 5000,
        });
        setTimeout(() => this.router.navigate(['/login']), 3000);
      } else {
        this.msg.add({ severity: 'error', summary: 'Error al registrar', detail: result.msg });
      }
    } catch (err: any) {
